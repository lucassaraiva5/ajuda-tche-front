                            ...
                            <div class="px-1" x-data>
                                ...
                                <div class="grid w-full grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                                    ...
                                    <x-input-group-inline wrapperClass="md:col-span-2 min-h-[50px]" label="Já foi contemplado com o Programa Volta por Cima?">
                                        ...
                                        <x-radio-input wire:model="form.volta_por_cima_beneficiary" name="form.volta_por_cima_beneficiary" label="Não" value="false"/>
                                    </x-input-group-inline>
                                    <x-input-group-inline wrapperClass="md:col-span-2 min-h-[50px]" label="Beneficiário do Programa São Leo Mais Renda?">
                                        <x-radio-input wire:model="form.city_beneficiary" name="form.city_beneficiary" label="Sim" value="true"/>
                                        <x-radio-input wire:model="form.city_beneficiary" name="form.city_beneficiary" label="Não" value="false"/>
                                    </x-input-group-inline>
                                    <x-input-group-inline label="Acessa alguma cozinha social?">
                                        <x-radio-input wire:model.live="form.access_social_kitchen" name="form.access_social_kitchen" label="Sim" value="true"/>
                                        <x-radio-input wire:model.live="form.access_social_kitchen" name="form.access_social_kitchen" label="Não" value="false"/>
                                    </x-input-group-inline>
                                    <x-text-input wire:model="form.which_social_kitchen" name="form.which_social_kitchen" label="Qual Cozinha Social" placeholder="Nome da Cozinha Social" :disabled="$this->form->access_social_kitchen !== 'true'"/>
                                    <x-input-group-inline label="Possui alguma deficiência?">
                                        <x-radio-input wire:model.live="form.has_disability" name="form.has_disability" label="Sim" value="true"/>
                                        <x-radio-input wire:model.live="form.has_disability" name="form.has_disability" label="Não" value="false"/>
                                    </x-input-group-inline>
                                    <x-text-input wire:model="form.disability_type" name="form.disability_type" label="Tipo de deficiência" placeholder="Cegueira, cadeirante..." :disabled="$this->form->has_disability !== 'true'"/>
                                </div>
                                <div class="mt-16">
                                ...
                                <div class="mt-16">
                                    ...
                                    <div class="grid w-full grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                                        <x-input-group-inline wrapperClass="md:col-span-2 min-h-[40px]" label="Você declara que esteve em situação de:">
                                            <x-radio-input wire:model="form.declared_situation" name="form.declared_situation" label="Desabrigado" value="desabrigado"/>
                                            <x-radio-input wire:model="form.declared_situation" name="form.declared_situation" label="Desalojado" value="desalojado"/>
                                            <x-radio-input wire:model="form.declared_situation" name="form.declared_situation" label="Atingido" value="atingido"/>
                                        </x-input-group-inline>
                                        ...
                                        <x-input-group-inline wrapperClass="md:col-span-2" label="Situação do domicílio:">
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
                                                ...
                                                <x-checkbox-input wire:model="form.housing_status" name="form.housing_status" label="Danificação dos móveis/eletrodomésticos" value="Danificação dos móveis/eletrodomésticos"/>
                                                <x-checkbox-input wire:model="form.housing_status" name="form.housing_status" label="Danificação das paredes/pisos/estrutura/telhado" value="Danificação das paredes/pisos/estrutura/telhado"/>
                                                <x-checkbox-input wire:model="form.housing_status" name="form.housing_status" label="Perda de pertences de cama/mesa e banho" value="Perda de pertences de cama/mesa e banho"/>
                                    ...
                                    <div class="mt-12 md:mt-32 grid grid-cols-1 gap-y-5">
                                        ...
                                        <x-checkbox-input wire:model="form.agree_data_sent_to_the_government" name="form.agree_data_sent_to_the_government" label="Ao salvar, você concorda que seus dados serão disponibilizados ao Governo do Estado do Rio Grande do Sul." value="true" :required="true"/>
                                        <x-checkbox-input wire:model="form.agree_true_data" name="form.agree_true_data" label="DECLARO QUE AS INFORMAÇÕES POR MIM PRESTADAS NESTA FICHA SÃO A EXPRESSÃO DA VERDADE. A PRESENTE DECLARAÇÃO É FEITA SOB AS PENAS DA LEI, PORTANTO O DECLARANTE DE QUE, EM CASO DE FALSIDADE FICARÁ SUJEITO A SANÇÕES CRIMINAIS, CIVIS, ADMINISTRATIVAS PREVISTAS NA LEGISLAÇÃO PRÓPRIA." value="true" :required="true"/>
                                    </div>
                                </div>
